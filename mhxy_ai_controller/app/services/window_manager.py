...
class WindowManager:
    def enumerate_windows(self):
        ...
        def cb(hwnd,_):
            ...
            result.append(WindowInfo(hwnd,title,*rect))
    ...
    def get_info(self,hwnd=None,title=None):
        if hwnd and win32gui.IsWindow(hwnd):
            return WindowInfo(hwnd,win32gui.GetWindowText(hwnd),*win32gui.GetWindowRect(hwnd))
        return self.find_by_title(title)
    def activate(self,hwnd):
        if not hwnd or not win32gui.IsWindow(hwnd):return False
        try:
            win32gui.ShowWindow(hwnd,win32con.SW_RESTORE); win32gui.SetForegroundWindow(hwnd); time.sleep(.1); return True
