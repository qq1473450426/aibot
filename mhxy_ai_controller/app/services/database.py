...
class Database:
    ...
    def event(self,account_id,level,event,message):
        with self.lock,self.conn:self.conn.execute('INSERT INTO events(ts,account_id,level,event,message) VALUES(?,?,?,?,?)',(time.time(),account_id,level,event,message))
    def state(self,s):
